...
class PresenceController extends Controller
{
    ...
    public function downloadQrCodeImage()
    {
        $code = request('code');

        // Validasi parameter code
        if (!$code) {
            return response('Code parameter is required', 400);
        }

        // Validasi attendance exists
        $attendance = Attendance::query()->where('code', $code)->first();
        if (!$attendance) {
            return response("Attendance with code '$code' not found", 404);
        }

        try {
            // Generate QR code sebagai SVG dengan kualitas tinggi dan scannable
            $svgContent = \SimpleSoftwareIO\QrCode\Facades\QrCode::format('svg')
                ->size(600) // Ukuran lebih besar untuk kualitas lebih baik
                ->margin(2)  // Margin yang cukup untuk scanning
                ->errorCorrection('M') // Medium error correction untuk balance antara size dan reliability
                ->generate($code);

            $filename = 'qrcode-' . $code . '.svg';
    ...
    }    public function getQrCode(?string $code): string
    {
        if (!Attendance::query()->where('code', $code)->first())
            throw new NotFoundHttpException("Tidak ditemukan absensi dengan code '$code'.");
